...
import { cars, vehicleTypes } from '@/lib/data';
...
export default function CarsByTypePage() {
  const params = useParams();
  const typeSlug = params.type as string;

  const typeInfo = vehicleTypes.find((t) => t.id === typeSlug);
  ...
  const filteredCars = useMemo(() => {
    let result = [...cars].filter((c) => c.type === typeSlug);
  ...
  const handleRemoveFilter = (key: string) => {
    if (key.startsWith('brand-')) {
      setFilters((f) => ({
        ...f,
        brands: f.brands.filter((b) => b !== key.replace('brand-', '')),
      }));
    } else if (key === 'price') {
  ...
  return (
    <div className="min-h-dvh bg-obsidian pt-[72px]">
      <div className="max-w-[1440px] mx-auto px-5 sm:px-6 lg:px-8 py-6 sm:py-8">
        {typeInfo && (
          <div className="mb-5 sm:mb-6" dir="rtl">
            <h1 className="text-xl sm:text-2xl font-bold text-cream">
              <span className="gold-gradient-text">{typeInfo.name}</span>
              <span className="text-muted text-base sm:text-lg mr-3 font-normal">— {typeInfo.nameEn}</span>
            </h1>
          </div>
        )}
            ...
            {paginatedCars.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 sm:py-24 text-center">
                ...
                <p className="text-sm text-muted max-w-sm">
                  لم يتم العثور على سيارات من نوع &quot;{typeInfo?.name ?? typeSlug}&quot; تطابق معايير البحث. حاول تعديل الفلاتر.
                </p>
              </div>
            )}
